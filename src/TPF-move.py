...
class SwarmNode(Node):
    ...
    def callback_vel(self,U,theta):
        U = np.array(U)
        U -= np.mean(U,axis = 0)
        #U += (np.cos(self.time/1e3)*self.dt,np.sin(self.time/1e3)*2*self.dt) # Elipse
        U += ((3*np.cos(self.time/1e3)+2*np.cos(self.time/1.5e3))*self.dt,
              (3*np.sin(self.time/1e3)-2*np.sin(self.time/1.5e3))*self.dt) # Integral da hipocicloide
        # assert self.classes == 4
        # U += np.tile([[np.cos(self.time/1e3)/6,np.sin(self.time/1e3)/6],
        #               [np.cos(self.time/5e2)/6,np.sin(self.time/5e2)/6],
        #               [np.cos(self.time/1e3)/3,np.sin(self.time/1e3)/3],
        #               [np.sin(self.time/1e3)/2,np.cos(self.time/1e3)/2]],
        #              (self.robots//self.classes,1)) # Cada classe um circulo distinto
